...
function url($url = null){
	$url_utama = "http://localhost/ramadani_laundry";
	if ($url != null) {
...
function add_kary($karyawan) {
    ...
    error_log("Nama: $nama, Username: $username, Email: $email, Level: $level");
    ...
    $insert = "INSERT INTO master (nama, email, username, password, level) VALUES ('$nama', '$email', '$username', '$password', '$level')";
...
function add_ck($data_ck) {
    ...
    $nama_pkt_ck = mysqli_real_escape_string($koneksi, htmlspecialchars($data_ck['nama_paket_ck']));
    $waktu_krj_ck = mysqli_real_escape_string($koneksi, htmlspecialchars($data_ck['waktu_kerja_ck']));
    $qty_ck = mysqli_real_escape_string($koneksi, htmlspecialchars($data_ck['kuantitas_ck']));
    $tarif_ck = mysqli_real_escape_string($koneksi, htmlspecialchars($data_ck['tarif_ck']));

    $query_ck = "INSERT INTO tb_cuci_komplit (nama_paket_ck, waktu_kerja_ck, kuantitas_ck, tarif_ck) VALUES (
...
function add_dc($data_dc) {
    ...
    $query_dc = "INSERT INTO tb_dry_clean (nama_paket_dc, waktu_kerja_dc, kuantitas_dc, tarif_dc) VALUES (
...
function add_cs($data_cs) {
    ...
    $query_cs = "INSERT INTO tb_cuci_satuan (nama_cs, waktu_kerja_cs, kuantitas_cs, tarif_cs, keterangan_cs) VALUES (
...
function order_ck($order_ck){
    ...
    if (mysqli_num_rows($pkt_ck) === 1) {
        ...
        $wkt_krj_ck = $result_ck['waktu_kerja_ck'];
        $harga_perkilo = $result_ck['tarif_ck'];  
        $tot_bayar = $berat_qty * $harga_perkilo;

        $str = uniqid();
        $limitNum = substr($str, 0, 7);
        $or_ck_number = 'CK-' . strtoupper($limitNum);
    }
    
    $insert_ck = "INSERT INTO tb_order_ck 
    (or_ck_number, nama_pel_ck, no_telp_ck, alamat_ck, jenis_paket_ck, wkt_krj_ck, berat_qty_ck, harga_perkilo, tgl_masuk_ck, tgl_keluar_ck, tot_bayar, keterangan_ck) 
    VALUES 
    ('$or_ck_number','$nama_pel','$no_telp','$alamat','$jns_pkt','$wkt_krj_ck','$berat_qty','$harga_perkilo','$tgl_masuk','$tgl_keluar','$tot_bayar','$ket')";
...
function order_dc($order_dc){
    ...
    $query_dc = "INSERT INTO tb_order_dc 
    (or_dc_number, nama_pel_dc, no_telp_dc, alamat_dc, jenis_paket_dc, wkt_krj_dc, berat_qty_dc, harga_perkilo, tgl_masuk_dc, tgl_keluar_dc, tot_bayar, keterangan_dc) 
    VALUES 
    ('$orderNum_dc', '$nama_pel_dc', '$no_telp', '$alamat_dc', '$jns_paket', '$wkt_kerja_dc', '$berat_dc', '$trf_dc', '$tgl_msk_dc', '$tgl_kel_dc', '$tot_bayar', '$ket_dc')";
...
function order_cs($order_cs) {
    ...
    $query_cs = "INSERT INTO tb_order_cs 
    (or_cs_number, nama_pel_cs, no_telp_cs, alamat_cs, jenis_paket_cs, wkt_krj_cs, jml_pcs, harga_perpcs, tgl_masuk_cs, tgl_keluar_cs, tot_bayar, keterangan_cs) 
    VALUES 
    ('$orderNum_cs', '$nama_pel_cs', '$no_telp_cs', '$alamat_cs', '$jenis_pkt_cs', '$wkt_krj_cs', '$jml_pcs', '$trf_cs', '$tgl_msk_cs', '$tgl_kel_cs', '$tot_bayar', '$ket_cs')";
...
function transaksi_ck($trans_ck) {
    ...
    (or_number, pelanggan, no_telp, alamat, j_paket, wkt_kerja, berat, h_perkilo, tgl_msk, tgl_klr, total, nominal_byr, kembalian, status, keterangan) 
    VALUES 
    ('$or_number', '$pelanggan', '$no_telp', '$alamat', '$jns_paket', '$wkt_kerja', '$berat', '$h_perkilo', '$tgl_msk', '$tgl_klr', '$total', '$nominal_bayar', '$kembalian', '$status', '$keterangan')";
...
function transaksi_cs($trans_cs) {
    ...
    $query_insert_cs = "INSERT INTO tb_riwayat_cs 
    (or_number, pelanggan, no_telp, alamat, j_paket, wkt_kerja, jml_pcs, h_perpcs, tgl_msk, tgl_klr, total, nominal_byr, kembalian, status, keterangan) 
    VALUES 
    ('$or_number', '$pelanggan', '$no_telp', '$alamat', '$jns_paket', '$wkt_kerja', '$jml_pcs', '$h_perpcs', '$tgl_msk', '$tgl_klr', '$total', '$nominal_bayar', '$kembalian', '$status', '$keterangan')";
...
function transaksi_dc($trans_dc) {
    ...
    $query_insert_dc = "INSERT INTO tb_riwayat_dc 
    (or_number, pelanggan, no_telp, alamat, j_paket, wkt_kerja, berat, h_perkilo, tgl_msk, tgl_klr, total, nominal_byr, kembalian, status, keterangan) 
    VALUES 
    ('$or_number', '$pelanggan', '$no_telp', '$alamat', '$jns_paket', '$wkt_kerja', '$berat', '$h_perkilo', '$tgl_msk', '$tgl_klr', '$total', '$nominal_bayar', '$kembalian', '$status', '$keterangan')";
